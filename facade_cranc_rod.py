...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def widgetAdjust(self):
        self.textInAngleGuid.setToolTip(
                'Угол направляющей ползуна в градусах (целое цисло)')
        self.textInAngleCrank.setToolTip(
                'Угол поворота кривошипа в градусах (целое число)')
        self.textInEccentrGuid.setToolTip(
                'Смещение направляющей ползуна в метрах')
        self.textInRotateSpeed.setToolTip(
                'Угловая скорость кривошипа в 1 / с' +
                ' (положительна если движение по часовой стрелке)')
        self.textInLengthCrank.setToolTip(
                'Длина кривошипа в метрах (только положительное число)')
        self.textInLengthRod.setToolTip(
                'Длина шатуна в метрах (только положительное число)')
    ...
    def readData(self):
        ...
        CSM = CrankSlideMech(
            angleGuid=float(self.textInAngleGuid.toPlainText()),
    ...
    def logToFile(self, message):
        ...
        logMessage = (
            f'{dt.now():{time_format}}\n'+
            f'Введенные значения параметров механизма:\n'+
            f'Угол наклона направляющей'+
            f'{self.textInAngleGuid.toPlainText()} \n'+
            ...
            f'Угловая скорость кривошипа'+
            f'{self.textInRotateSpeed.toPlainText()}\n'+
    ...
    def plotPosition(self):
        ...
        if  (minDistanceMech < distanceToGuid or CSM.lengthCrank == 0 
            or CSM.lengthRod == 0):
            self.labelLog.setText(
                f'Механизм с такими параметрами\n'+
                f'не существует.\n'+
                f'Проверьте корректность\n'+
                f'введенных данных.')
            errMessage = ('Ошибка:\n'+
        ...
        test_eq1 = (x_B * np.tan(CSM.angleGuid) + 
            CSM.eccentrGuid / np.cos(CSM.angleGuid) - y_B)
        ...
        if test_eq1 > acceptError or test_eq2 > acceptError:
            ...
            errMessage = ('Ошибка:\nОшибка решения уравнений при расчете ' +
                'координат механизма.\n\n')
            self.logToFile(errMessage)
            return
        ...
        yPointsGuid = [
            ((x_B - CSM.lengthRod / 2) * np.tan(CSM.angleGuid) + 
            CSM.eccentrGuid / np.cos(CSM.angleGuid)), 
            ((x_B + CSM.lengthRod / 2) * np.tan(CSM.angleGuid) + 
            CSM.eccentrGuid / np.cos(CSM.angleGuid))
            ]
        self.axes.plot(xPointsGuid, yPointsGuid, '--', c='orange')
        #Отрисовка стойки
        xPointsBody = [0, 0.3 * CSM.lengthCrank * np.sin(np.pi / 6), 
            -0.3 * CSM.lengthCrank * np.sin(np.pi / 6), 0]
        ...
        rotX1 = (x_B + (x1 - x_B) * np.cos(CSM.angleGuid) - 
            (y1 - y_B) * np.sin(CSM.angleGuid))
        rotX2 = (x_B + (x2 - x_B) * np.cos(CSM.angleGuid) - 
            (y2 - y_B) * np.sin(CSM.angleGuid))
        rotX3 = (x_B + (x3 - x_B) * np.cos(CSM.angleGuid) - 
            (y3 - y_B) * np.sin(CSM.angleGuid))
        rotX4 = (x_B + (x4 - x_B) * np.cos(CSM.angleGuid) - 
            (y4 - y_B) * np.sin(CSM.angleGuid))
        rotY1 = (y_B + (x1 - x_B) * np.sin(CSM.angleGuid) + 
            (y1 - y_B) * np.cos(CSM.angleGuid))
        rotY2 = (y_B + (x2 - x_B) * np.sin(CSM.angleGuid) + 
            (y2 - y_B) * np.cos(CSM.angleGuid))
        rotY3 = (y_B + (x3 - x_B) * np.sin(CSM.angleGuid) + 
            (y3 - y_B) * np.cos(CSM.angleGuid))
        rotY4 = (y_B + (x4 - x_B) * np.sin(CSM.angleGuid) + 
            (y4 - y_B) * np.cos(CSM.angleGuid))
        ...
        plt.xlim(minCoord - 0.2 * max(abs(maxCoord), abs(minCoord)),
                 maxCoord + 0.2 * max(abs(maxCoord), abs(minCoord)))
        plt.ylim(minCoord - 0.2 * max(abs(maxCoord), abs(minCoord)),
                 maxCoord + 0.2 * max(abs(maxCoord), abs(minCoord)))
        self.axes.annotate('O', (0.1 * CSM.lengthCrank, 0.1 * CSM.lengthCrank))
        self.axes.annotate('A', (x_A + 0.1 * CSM.lengthCrank, 
            y_A + 0.1 * CSM.lengthCrank))
        self.axes.annotate('B', (x_B + 0.1 * CSM.lengthCrank, 
            y_B + 0.25 * CSM.lengthCrank))
    ...
    def plotSpeed(self):
        ...
        distanceToGuid = (abs(CSM.eccentrGuid / np.cos(CSM.angleGuid) / 
                         np.sqrt(1 + np.tan(CSM.angleGuid)**2)))
        minDistanceMech = (CSM.lengthRod - CSM.lengthCrank)
        if  (minDistanceMech < distanceToGuid or CSM.lengthCrank == 0 or 
            CSM.lengthRod == 0):
            self.labelLog.setText(
                f'Механизм с такими параметрами\n'+
                f'не существует.\n'+
        ...
        self.axes.quiver(0, 0, xLenVe_A, yLenVe_A, 
            units='xy', scale=1, color='r')
        self.axes.quiver(xPointV_A, yPointV_A, xLenVe_BA, yLenVe_BA, 
            units='xy', scale=1, color='g')
        self.axes.quiver(0, 0, xLenVe_B, yLenVe_B, 
            units='xy', scale=1, color='b')
        self.axes.plot([0, xPointV_A], [0, yPointV_A], c='r', label='v_A')
        self.axes.plot([xPointV_A, xPointV_B], [yPointV_A, yPointV_B], 
            c='g', label='v_BA')
        ...
        plt.xlim(minVeh - 0.2 * max(abs(maxVeh), abs(minVeh)), 
            maxVeh + 0.2 * max(abs(maxVeh), abs(minVeh)))
        plt.ylim(minVeh - 0.2 * max(abs(maxVeh), abs(minVeh)), 
            maxVeh + 0.2 * max(abs(maxVeh), abs(minVeh)))
        self.fig.canvas.draw()

        self.labelLog.setText(
            f'Скорости точек механизма:\n' +
            f'Cкорость точки А:\nv_А={abs(v_A):.2f} м/c.\n' +
            f'Cкорость точки В:\nv_В={abs(v_B):.2f} м/c.\n' +
    ...
    def plotAcceleration(self):
        ...
        angleAccRod = (np.arctan((yPointA_B - yPointA_A) /
            (xPointA_B - xPointA_A)))
        ...
        #Костыль.
        #Иногда угол определяется с ошибкой в 180 градусов.
        #В чем проблема не совсем понятно.
        isAngleAccRodTrue = (xPointA_B -
            (xPointA_A + xLenA_BA * np.cos(CSM.angleGuid)) < 0.001)
        if not isAngleAccRodTrue:
            xLenA_BA = a_BA * np.cos(angleAccRod)
            yLenA_BA = a_BA * np.sin(angleAccRod)
        
        self.axes.plot([0, xPointA_A], [0, yPointA_A], 
            c='r', label='a_A')
        self.axes.plot([xPointA_A, xPointA_N], [yPointA_A, yPointA_N], '--', 
            c='g')
        self.axes.plot([xPointA_N, xPointA_B], [yPointA_N, yPointA_B], '--', 
            c='g')
        self.axes.plot([xPointA_A, xPointA_B], [yPointA_A, yPointA_B], 
            c='g', label='a_BA')
        self.axes.plot([0, xPointA_B], [0, yPointA_B], c='b', label='a_B')
        self.axes.quiver(0, 0, xLenA_A, yLenA_A, units='xy', scale=1, 
            color='r')
        self.axes.quiver(xPointA_A, yPointA_A, xLenA_BA, yLenA_BA, 
            units='xy', scale=1, color='g')
        self.axes.quiver(0, 0, xLenA_B, yLenA_B, 
            units='xy', scale=1, color='b')
        #Параметры графика
        minAcc = min(xPointA_A, xPointA_B, xPointA_N, yPointA_A, 
            yPointA_B, yPointA_N)
        maxAcc = max(xPointA_A, xPointA_B, xPointA_N, yPointA_A, 
            yPointA_B, yPointA_N)
        plt.xlim(minAcc - 0.2 * max(abs(maxAcc), abs(minAcc)), 
            maxAcc + 0.2 * max(abs(maxAcc), abs(minAcc)))
        plt.ylim(minAcc - 0.2 * max(abs(maxAcc), abs(minAcc)), 
            maxAcc + 0.2 * max(abs(maxAcc), abs(minAcc)))
        self.axes.grid(True, c='lightgrey', alpha=0.5)
        self.axes.set_aspect('equal')
...
def plotSingleEmptyGraph():
    ...
    fig, axes = plt.subplots(
        nrows=1, ncols=1, figsize=(4.98, 5.19), dpi=85, 
        facecolor='white', frameon=True, edgecolor='black', linewidth=1
        )
    fig.subplots_adjust(wspace=0.4, hspace=0.4, left=0.15, right=0.85, 
        top=0.9, bottom=0.1)
